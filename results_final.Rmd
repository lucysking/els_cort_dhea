...
severity_file <- "~/Desktop/ELS/stress_data/data/severity_by_event_20190825.csv"
...
ggsave("~/Desktop/ELS/cort_dhea/cort_dhea_sync/interval_dist.png", width = 7, height = 5)
```


```{r tanner_dis, include=FALSE}
cd_clean %>% 
...
ggsave("~/Desktop/ELS/cort_dhea/cort_dhea_sync/tanner_dist.png", width = 7, height = 5)
...
cd_wf_clean %>% 
  ...
  labs(
    x = "Change in pubertal stage"
  )

ggsave("~/Desktop/ELS/cort_dhea/cort_dhea_sync/tanner_change.png", width = 7, height = 5)

cd_clean <-
  cd_clean %>% 
  left_join(
    cd_wf_clean %>% 
      select(ELS_ID, tan_diff),
    by = "ELS_ID"
  )
...
```{r t1t2_mod_sens_tan, include=FALSE}
cd_clean_tan <-
  cd_clean %>% 
  filter(tan_diff > 0)

contrasts(cd_clean_tan$timepoint) = c(0, 1) # T1 is baseline

t1t2_mod1_tp_tan <- lmer(
  scale(cort_clean) ~ 
    scale(dhea_clean) * timepoint +
    (1 | ELS_ID),
  data = cd_clean_tan
...
summary(t1t2_mod1_tp_tan)
```

## Associations between DHEA and cortisol in earlier versus later puberty: interval in years 
...
cd_clean <-
  ...
  mutate(
    time = case_when(
      timepoint == "T1" ~ 0,
      timepoint == "T2" ~ interval_yr
    )
  )
```

```{r t1t2mod_t2_interval, warning=FALSE, include=FALSE}
t1t2_mod1_interval <- lmer(
  scale(cort_clean) ~ 
    scale(dhea_clean) * scale(time) +
    (1 | ELS_ID),
  data = cd_clean
)

summary(t1t2_mod1_interval)

#confint.merMod(t1t2_mod1_interval, method = "boot")
```
...
```{r t1t2_mod_sens_test, include=FALSE}
contrasts(cd_clean_tan$timepoint) = c(0, 1) # T1 is baseline

t1t2_mod1_tp_tan_test <- lmer(
  scale(cort_clean) ~ 
    scale(test_clean) * timepoint +
    (1 | ELS_ID),
  data = cd_clean_tan
...
summary(t1t2_mod1_tp_tan_test)
```

## Associations between testosterone and cortisol in earlier versus later puberty: interval in years 

```{r t1t2mod_t2_interval_test, warning=FALSE, include=FALSE}
t1t2_mod1_interval_test <- lmer(
  scale(cort_clean) ~ 
    scale(test_clean) * scale(time) +
    (1 | ELS_ID),
  data = cd_clean
)

summary(t1t2_mod1_interval_test)

#confint.merMod(t1t2_mod1_interval_test, method = "boot")
...
```{r couple_me_sensitivity_tan, include=FALSE}
cd_wf_clean_tan <-
  cd_wf_clean %>% 
  filter(tan_diff > 0)

coupling_mod_tan_me <- lm(
  scale(cort_slope_yr) ~
    scale(dhea_slope_yr_win) +
    scale(cort_clean_t1) +
    scale(dhea_clean_t1),
  data = cd_wf_clean_tan
)

summary(coupling_mod_tan_me)
```
...
```{r couple_me_sensitivity_tan_test, include=FALSE}
coupling_mod_tan_me_test <- lm(
  scale(cort_slope_yr) ~
    scale(test_slope_yr_win) +
    scale(cort_clean_t1) +
    scale(test_clean_t1),
...
summary(coupling_mod_tan_me_test)
```
...
```{r coupling_tan, include=FALSE}
coupling_mod1_tan <- lm(
  scale(cort_slope_yr) ~ 
    scale(dhea_slope_yr_win) * 
    scale(sumsev_threat_t1) +
    scale(cort_clean_t1) +
...
## Association of non-threat severity with longitudinal coupling of DHEA and cortisol across the pubertal transition
```{r include=FALSE}
cd_wf_clean <-
  cd_wf_clean %>% 
  mutate(
...
```{r coupling_tan_test, include=FALSE}
coupling_mod1_tan_test <- lm(
  scale(cort_slope_yr) ~ 
    scale(test_slope_yr_win) * 
    scale(sumsev_threat_t1) +
    scale(cort_clean_t1) +
...
summary(coupling_mod1_tan_test)
```
...
ggsave("~/Desktop/ELS/cort_dhea/cort_dhea_sync/coupling_test.png")
```

## Association of non-threat severity with longitudinal coupling of testosterone and cortisol across the pubertal transition
```{r include=FALSE}
