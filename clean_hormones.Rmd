...
#Files
hormone_info_file <- "/Volumes/mooddata/ELS_RDoC/Saliva Sampling/ELS Hormone Master Sheet.xlsx"
sex_file <- "/Volumes/mooddata/ELS_RDoC/Saliva Sampling/els_t1_biological_sex.csv"

cortisol_files  <-
  tibble(
    path = 
      list.files(
        path = "/Volumes/mooddata/ELS_RDoC/Saliva Sampling/Hormones/Results/cortisol",
        pattern = "^Raw_",
        full.names = TRUE
      )
  )
...
dhea_files  <-
  tibble(
    path = 
      list.files(
        path = "/Volumes/mooddata/ELS_RDoC/Saliva Sampling/Hormones/Results/DHEA",
        pattern = "^Raw_",
        full.names = TRUE
      )
  )
...
testosterone_files  <-
  tibble(
    path = 
      list.files(
        path = "/Volumes/mooddata/ELS_RDoC/Saliva Sampling/Hormones/Results/testosterone",
        pattern = "^Raw_",
        full.names = TRUE
      )
  )
...
estradiol_files  <-
  tibble(
    path = 
      list.files(
        path = "/Volumes/mooddata/ELS_RDoC/Saliva Sampling/Hormones/Results/estradiol",
        pattern = "^Raw_",
        full.names = TRUE
      )
  )
...
progesterone_files  <-
  tibble(
    path = 
      list.files(
        path = "/Volumes/mooddata/ELS_RDoC/Saliva Sampling/Hormones/Results/progesterone",
        pattern = "^Raw_",
        full.names = TRUE
      )
  )
...
write_csv(
  hormones_final, 
  "~/Desktop/ELS/cort_dhea/data/data_final/hormones_final_20190506.csv"
) 

hormones_final %>% 
  filter(timepoint == "T1") %>% 
  filter(!is.na(dhea_mean_clean) | !is.na(cortisol_mean_clean)) %>% 
  ungroup() %>% 
